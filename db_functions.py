...
import sqlite3
from ui_functions import center_window
...
#had issues with columns being added after the database was created. 
...
#CLEARING DATABASE FOR DEBUGGING 
def clear_database():
    cursor = db_connection.cursor()
...
# save task content to the database --> initial creation of a task
...
# Update task content in the database --> save function
...
def load_tasks_for_day(day):
    cursor = db_connection.cursor()
    #query gets all non-completed tasks
...
################################################## TEXT EDITOR WINDOW ##################################################
# function to open text editor window for a task
def open_text_editor(task_title, task_id=None, day=None, status="not-complete"):
    ...
    # save the task content --> for the save button
    def save_content():
        nonlocal task_id
    ...
    # Delete the task function --> for the delete function
    def delete_content():
        if messagebox.askyesno("Confirm Delete", f"Are you sure you want to delete the task '{task_title}'?"):
            ...
            if task_title in task_buttons[day]:
                task_buttons[day][task_title].destroy() #removing the task-button from weekly view
                del task_buttons[day][task_title]
            messagebox.showinfo("Deleted", f"Task '{task_title}' deleted!")
            editor_window.destroy()

    # Mark task as completed function --> for completed button
    ...
    # Buttons for save, delete, edit, and mark as completed - connect to the database
